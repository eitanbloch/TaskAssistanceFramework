...
class UR3e_PARAMS(object):
    """
    UR3e_PARAMS determines physical costants for UR5e Robot manipulator
    @param inflation_factor - by what factor inflate the minimal sphere radious of each link
    @param tool_lenght - the lenght of the tool [meters]. for the gripper, set 0.135 meter
    """
    ...
    def __init__(self, inflation_factor=1.0, tool_lenght=0.135):
        # alpha, a, d, theta_const
        self.ur_DH = [[0, 0, 0.15185, 0],  # shoulder - > base_link
                      [np.pi / 2, 0, 0, 0],  # upper_arm_link - > shoulder
                      [0, -0.24355, 0, 0],  # forearm_link -> upper_arm_link
                      [0, -0.2132, 0.13105, 0],  # wrist1 -> forearm_link
                      [np.pi / 2, 0, 0.08535, 0],  # wrist2 -> wrist1
                      [-np.pi / 2, 0, 0.0921, 0]]  # wrist3 -> wrist2

        # relative to the former
        self.ur_geometry = [['shoulder_link', np.array([0, 0, -0.105]), 'z'],
                            ['upper_arm_link', np.array([-0.25, 0, 0.095]), 'x'],
                            ['upper_arm_link', np.array([0, 0, 0.095]), 'z'],
                            ['forearm_link', np.array([0, 0, 0.095]), 'z'],
                            ['forearm_link', np.array([-0.225, 0, 0.015]), 'x'],
                            ['wrist_1_link', np.array([0, 0, -0.099]), 'z'],
                            ['wrist_1_link', np.array([0, -0.09, 0]), 'y'],
                            ['wrist_1_link', np.array([0, 0.03, 0]), 'y'],
                            ['wrist_2_link', np.array([0, 0.0625, 0]), 'y'],
                            ['wrist_2_link', np.array([0, -0.035, 0]), 'y'],
                            ['wrist_3_link', np.array([0, 0, tool_lenght]), 'z']]
...
class UR5e_camera_PARAMS(object):
    """
    UR5e_PARAMS determines physical costants for UR5e Robot manipulator
    @param inflation_factor - by what factor inflate the minimal sphere radious of each link
    @param tool_lenght - the lenght of the tool [meters]. for the gripper, set 0.135 meter
    ...
    def __init__(self, inflation_factor=1.0, tool_lenght=0.135):
        ...
        # relative moves to the center of the former !!!!!
        self.ur_geometry = [['shoulder_link', np.array([0, 0, -0.1625]), 'z'],
                            ['upper_arm_link', np.array([-0.425, 0, 0.135]), 'x'],
                            ['upper_arm_link', np.array([0, 0, 0.135]), 'z'],
                            ['forearm_link', np.array([0, 0, 0.135]), 'z'],
...
class Transform(object):
    ...
    def __init__(self, ur_params, is_camera=False):
        ...
        for frame, offset, axis in self.robot_geometry:
            ...
            for sphere_offset in local_sphere_offset:
                if axis == 'x':
                    self.local_sphere_coords[frame].append(
                        np.array([sphere_offset, offset[1], offset[2], 1], dtype=float))
                elif axis == 'y':
                    self.local_sphere_coords[frame].append(
                        np.array([offset[0], sphere_offset, offset[2], 1], dtype=float))
                else:
                    self.local_sphere_coords[frame].append(
                        np.array([offset[0], offset[1], sphere_offset, 1], dtype=float))
        self.x_translation = 0

    def make_camera(self, x_translation=0.5):
        """
        Make the camera transformation
        """
        self.camera = True
