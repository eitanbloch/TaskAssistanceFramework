...
class Environment(object):
    ...
    def __init__(self, env_idx):
        ...
        if env_idx == 1:
            ...
            self.wall_x_const(y_min=-0.58, y_max=0, z_min=0, z_max=0.45, x_const=0.6, obstacles=obstacles)
        elif env_idx == 2:
            x_const = 0
            ...
            self.wall_x_const(-0.12, 0.12, 0, 0.15, 0.35, obstacles)
            self.wall_x_const(-0.12, 0.12, 0, 0.15, 0.70, obstacles)

            self.wall_x_const(-0.42, -1.02, 0, 0.24, 0.2, obstacles)
            self.wall_x_const(-0.42, -1.02, 0.48, 0.72, 0.2, obstacles)

            # window
            self.wall_x_const(-0.42, -0.54, 0.24, 0.48, 0.2, obstacles)
            self.wall_x_const(-0.78, -1.02, 0.24, 0.48, 0.2, obstacles)

            # window wall of the inner room
            self.wall_y_const(-1, 1, 0, 0.2, 0.4, obstacles)
